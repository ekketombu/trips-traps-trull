...
class TicTacToe:
    def __init__(self):
        self.board = [' ' for _ in range(9)]  # Representing 3x3 grid
    ...
    while game.empty_squares():
        ...
        if game.make_move(square, letter):
            ...
            if game.current_winner:
                if print_game:
                    print(letter + ' wins!')
                return letter  # Return the winner letter (X or O) if there is one
...
class SmartComputerPlayer:
    ...
    def minimax(self, state, player):
        ...
        if player == max_player:
            best = {'position': None, 'score': -float('inf')}  # Each score should maximize (be larger)
        else:
